...
위의 요소를 `tag`, `class`, `id`로 타겟팅하는 것은 매우 불안정하고 변경되기 쉽습니다. 요소를 교체하거나, CSS를 리팩터링하고 ID를 갱신하거나, 요소의 스타일에 영향을 주는 클래스를 추가/제거할 수 있기 때문입니다.
...
`Submit`이라는 단어가 중요하며 변경되면 안 되면 `cy.contains()`를 이용해 요소를 타겟팅합니다.
