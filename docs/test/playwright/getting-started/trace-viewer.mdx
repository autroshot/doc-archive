...
플레이라이트 추적 뷰어(Trace Viewer)는 테스트의 기록된 추적을 탐색할 수 있는 GUI 도구입니다. 테스트에서 각 작업의 앞뒤로 이동하고 작업 중에 발생한 일을 시각적으로 확인할 수 있습니다.
...
<Image img={htmlReportDetailedView} alt={'HTML 보고서 상세 보기'} />

보고서에 대한 자세한 내용은 [HTML 보고서](https://playwright.dev/docs/test-reporters#html-reporter)를 포함한 보고서 상세 안내서를 확인하세요.

## 추적 보기

각 작업을 클릭하거나 타임라인에 마우스를 놓아 테스트 추적을 보고 작업 전후의 페이지 상태를 확인합니다. 테스트의 각 단계에서 로그, 소스, 네트워크를 검사합니다. 추적 뷰어는 DOM 스냅숏을 생성하므로 완전한 상호 작용이 가능하고 개발자 도구를 여는 등의 작업이 가능합니다.
