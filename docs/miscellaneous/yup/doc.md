...
원한다면 타입스크립트의 인터페이스 병합 동작을 사용하여 스키마 타입을 확장할 수 있습니다. 타입 확장은 `globals.d.ts`와 같은 주변 타입 정의 파일에 있어야 합니다. 앱 코드에서 엽 타입을 실제로 확장하는 것을 잊지 말아야 합니다.
...
엽은 다국어를 지원합니다. `setLocale` 함수는 번역 키와 값으로 오류 객체를 생성하는 데 사용할 수 있는 함수를 받습니다. 이것들은 원하는 i18n 라이브러리에 넣을 수 있습니다.
