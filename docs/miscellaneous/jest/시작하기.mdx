...
이 테스트는 `expect`와 `toBe`를 사용하여 두 값이 정확히 일치하는지 확인합니다. 제스트가 테스트할 수 있는 다른 항목에 대해 알아보려면 [매처 사용하기](./매처-사용하기.md)를 참고하세요.
...
명령 행으로 `jest`를 실행하는 방법에 대해 더 알고 싶다면 [제스트 CLI 옵션](https://jestjs.io/docs/29.1/cli) 페이지를 참고하세요.
