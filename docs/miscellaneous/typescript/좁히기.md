...
`padding`에 `number`를 건네줄 때의 로직을 구현해 보겠습니다.
...
`padding`에 오류가 발생합니다. 타입스크립트는 `number`에 `number | string`을 할당하는 것을 경고합니다. 즉, `padding`이 `number`인지 명시적으로 확인하거나, `padding`이 `string`인 경우를 처리하지 않았습니다.
...
이 코드는 특별할 게 없는 자바스크립트 코드처럼 보이는데, 그 점이 핵심입니다. 타입스크립트의 타입 시스템은 타입 안정성을 위해 큰 변화가 필요하지 않습니다. 일반적인 자바스크립트 코드처럼 쉽게 작성하는 것을 목표로 합니다.

일반적인 자바스크립트처럼 보이지만 코드 뒤에서는 많은 일이 벌어지고 있습니다. 타입스크립트가 정적 타입을 사용하여 런타임 값을 분석하는 것과 유사하게 타입스크립트는 `if/else`, 조건부 삼항, 반복문, 참 같은 값 확인 같이 타입에 영향을 주는 자바스크립트 런타입 제어 흐름 구조에서 타입 분석을 덮어씌웁니다.

`if`문에서 타입스크립트는 `typeof padding === "number"`를 **타입 가드(type guard)**라는 특별한 형태의 코드로 이해합니다. 타입스크립트는 해당 위치에서 가능한 구체적인 타입을 얻을 수 있는 실행 경로를 따릅니다. 이러한 특별 검사(타입 가드), 할당, 그리고 타입을 선언된 것보다 구체적인 타입으로 정제하는 과정을 **좁히기**라고 부릅니다. 많은 편집기에서 이렇게 타입이 변경될 때 이를 관찰할 수 있으며 예시에서도 그렇게 할 것입니다.
...
`padLeft`에서 봤듯이 이 연산자는 자바스크립트 라이브러리에 자주 등장합니다. 타입스크립트는 이를 다른 분기의 좁은 타입으로 이해할 수 있습니다.

타입스크립트에서는 `typeof`에서 반환된 값을 확인하는 것이 타입 가드입니다. 타입스크립트는 `typeof`가 다른 값에서 작동하는 방식을 그대로 인코딩하므로 자바스크립트의 일부 이상한 방식을 그대로 갖습니다. 예를 들어, 위의 목록에서 `typeof`는 문자열 `null`을 반환하지 않습니다.
...
자바스크립트에서는 조건문에 `&&`, `||`, `if`문, 불린 부정(`!`) 같은 모든 표현식을 사용할 수 있습니다.
...
위의 값들은 `false`로 강제되고 나머지 값은 `true`로 강제됩니다. 값에 `Boolean` 함수를 사용하거나, 더 짧은 이중 불린 부정(`!!`)을 사용해서 값을 `boolean`으로 강제할 수 있습니다. 전자는 타입스크립트가 값을 `boolean` 타입으로 추론하는 반면, 후자는 좁은 리터럴 불린 타입인 `true`로 추론한다는 이점이 있습니다.
...
특히 `null`이나 `undefined` 같은 값을 차단할 때 이 특징을 많이 활용하곤 합니다.

예시로 해당 특징을 `printAll` 함수에서 활용해 보겠습니다.
...
`strs`이 참 같은 값인지 확인하여 앞의 오류를 제거했습니다. 이는 최소한 다음과 같은 두려운 오류를 방지합니다.
...
함수의 본문을 참 같은 값 확인으로 감쌌지만 여기에는 미묘한 단점이 존재합니다. 빈 문자열을 올바르게 처리하지 못한다는 것입니다.
...
위의 예시에서 `x`와 `y`가 동일한 것을 확인했을 때, 타입스크립트는 둘의 타입도 동일하다는 것을 압니다. `x`와 `y`의 유일한 공통 타입이 `string`이므로 타입스크립트는 첫 번째 분기의 `x`와 `y`가 `string`이라는 것을 압니다.

특정 리터럴 값(변수와 반대)에 대한 검사에서도 동일하게 작동합니다. 참 같은 값으로 좁히기 섹션에서, `printAll` 함수가 빈 문자열을 제대로 처리하지 않아서 오류가 발생하기 쉽다고 얘기했습니다. 대신 다음과 같이 `null`을 차단하는 특정 검사를 수행하여 타입스크립트가 `strs`의 타입에서 `null`을 올바르게 제거할 수 있습니다.
...
이 예시에서 `pet is Fish`가 타입 단언입니다. 단언은 `parameterName is Type`의 형식을 갖습니다. 여기서 `parameterName`은 현재 함수 시그니처의 매개변수 이름이어야 합니다.
...
원인지 사각형인지에 따라 올바른 로직을 적용하는 `getArea` 함수를 작성할 수 있습니다.
...
다음과 같이 새 멤버를 `Shape` 합집합에 추가하면 타입스크립트 오류가 발생합니다.
