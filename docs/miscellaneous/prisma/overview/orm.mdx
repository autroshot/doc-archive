...
예를 들어 새 프로젝트는 노드 마이그레이션 프레임워크 [db-migrate](https://github.com/db-migrate/node-db-migrate)를 사용하여 데이터베이스 스키마 및 마이그레이션을 생성하고 프리즈마 클라이언트를 쿼리에 사용할 수 있습니다. 자세한 내용은 [기존 데이터베이스가 이미 존재하는 워크플로](#기존-데이터베이스가-있는-프로젝트에서-점진적-채택을-하는-워크플로)를 확인하세요.
