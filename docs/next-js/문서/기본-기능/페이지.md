...
> **참고**
>
> 동적 라우팅에 대해 자세히 알아보려면 [동적 라우팅 문서](#)를 확인하세요.
...
CSR을 정적 생성, SSR과 함께 사용하는 것도 가능합니다. 페이지의 일부는 클라이언트 측 자바스크립트로 완전히 렌더링될 수 있습니다. 자세한 내용은 [데이터 가져오기](#) 문서를 참조하세요.
...
export async function getStaticProps() {
  ...
  // { props: { posts } }를 반환하면
  // Blog 컴포넌트는 빌드 타임에 posts를 프랍으로 받습니다.
  return {
    props: {
      posts,
    },
...
`getStaticProps`의 작동 방식에 대해 자세히 알아보려면 [데이터 가져오기 문서](#)를 확인하세요.
...
`getStaticPaths`의 작동 방식에 대해 자세히 알아보려면 [데이터 가져오기 문서](#)를 확인하세요.

> **참고**
>
> 정적 생성을 언제 사용해야 할지 궁금하다면 [사전 렌더링과 데이터 가져오기](#)를 확인하세요.
...
`getServerSideProps`의 작동 방식에 대해 자세히 알아보려면 [데이터 가져오기 문서](#)를 확인하세요.
