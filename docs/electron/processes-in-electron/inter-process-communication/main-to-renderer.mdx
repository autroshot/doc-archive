...
<Button href="https://fiddle.electronjs.org/launch?target=electron/v25.5.0/docs/fiddles/ipc/pattern-3" />
...
```javascript title="main.js (주 프로세스)" {10-25}
...
사전 로드 스크립트를 로드한 후, 렌더러 프로세스는 `window.electronAPI.onUpdateCounter()` 수신기 함수에 접근할 수 있어야 합니다.
...
이전 예시의 코드를 약간 수정하여 이를 시연해 보겠습니다. 렌더러 프로세스에서 `event` 매개변수를 사용하여 `counter-value` 채널을 통해 주 프로세스에 응답을 보냅니다.
