...
지원 파일은 모든 스펙 파일에 적용하고 사용할 수 있는 [사용자 정의 명령](https://docs.cypress.io/api/cypress-api/custom-commands)이나 전역 재정의 같은 재사용 가능한 동작을 넣기에 좋은 위치입니다.
...
파일 감시 동작에 대한 추가 제어가 필요한 경우에는 이 전처리기를 명시적으로 설정할 수 있습니다. 이는 감시 대상 설정이나, 변경 후 '업데이트' 이벤트 발생 전의 지연과 같은 동작을 설정하는 옵션을 제공합니다.
