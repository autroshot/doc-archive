...
다음 예시는 여러 요소에 대해 단언을 작성하는 사용 사례입니다. `.should()` 콜백 함수를 사용하면 부모의 여러 자식 요소에 대해 질의하고 무언가를 단언할 수 있습니다.

이렇게 하면 일반적인 사이프러스 DOM 명령을 사용하여 개별적으로 질의할 필요 없이, 하위 항목의 상태가 예상과 일치하도록 사이프러스를 차단하고 보호할 수 있습니다.
