...
[사전 기본 스타일](/docs/preflight)을 비활성화한 경우 콘텐츠 속성은 기본적으로 빈 문자열로 설정되지 않으며, `before` 및 `after` 수정자를 사용할 때마다 `content-['']`를 포함해야 한다는 점을 유의하세요.
...
테일윈드의 [테두리 재설정](/docs/preflight#border-styles-are-reset-globally)은 파일 입력 버튼에는 적용되지 않습니다. 즉, 파일 입력 버튼에 테두리를 추가하려면 [테두리 너비](/docs/border-width) 유틸리티와 함께 `file:border-solid`와 같은 클래스를 사용하여 [테두리 스타일](/docs/border-style)을 명시적으로 설정해야 합니다.
...
프로젝트에서 기본 `<dialog>` 요소를 사용한다면, `open` 수정자를 사용하는 [열기/닫기 상태에 스타일 지정하기](/docs/hover-focus-and-other-states#open-closed-state)를 읽어 보는 것도 좋습니다.
