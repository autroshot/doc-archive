...
- **변환기는 순수해야 합니다.** [상태 갱신 함수](https://react.dev/learn/queueing-a-series-of-state-updates)와 유사하게 변환기는 렌더링 중에 실행됩니다! (작업은 다음 렌더링까지 대기합니다.) 이는 변환기가 [순수](https://react.dev/learn/keeping-components-pure)해야 한다는 것을 의미합니다. 동일한 입력은 항상 동일한 출력을 생성해야 합니다. 요청을 보내거나, 타임아웃을 예약하거나, 부작용(컴포넌트 외부에 영향을 주는 작업)을 수행해서는 안 됩니다. 변형 없이 [객체](https://react.dev/learn/updating-objects-in-state)와 [배열](https://react.dev/learn/updating-arrays-in-state)을 갱신해야 합니다.
