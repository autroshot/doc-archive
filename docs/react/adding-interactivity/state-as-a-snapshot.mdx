...
클릭과 같은 사용자 이벤트에 대한 응답으로 사용자 인터페이스가 직접 변경된다고 생각할 수 있습니다. 리액트에서는 이 심성 모형(mental model)과 조금 다르게 작동합니다. 이전 페이지에서 리액트의 [상태 설정은 리렌더링을 요청한다](./render-and-commit.mdx#1단계-렌더링을-트리거한다)는 것을 확인했습니다. 즉, 인터페이스가 이벤트에 반응하려면 **상태 갱신**이 필요합니다.
...
[렌더링](./render-and-commit.mdx#2단계-리액트는-컴포넌트를-렌더링한다)은 리액트가 컴포넌트(함수)를 호출한다는 것을 의미합니다. 해당 함수에서 반환되는 JSX는 시간에 따른 UI의 스냅숏과 같습니다. 프롭, 이벤트 처리기, 지역 변수는 모두 **렌더링 시점의 상태를 사용하여** 계산되었습니다.
...
작은 실험을 통해 이것이 어떻게 작동하는지 알아보겠습니다. 다음 예시에서 `+3` 버튼을 클릭하면 `setNumber(number + 1)`가 세 번 호출되므로 카운터가 세 번 증가할 것이라고 예상할 수 있습니다.
...
이벤트 처리기의 코드가 비동기인 경우에도 **상태 변수의 값은 렌더링 내에서 절대 변경되지 않습니다**. **해당 렌더링**의 `onClick` 내부에서, `setNumber(number + 5)`가 호출된 후에도 `number`의 값은 계속 `0`입니다. 그 값은 리액트가 컴포넌트를 호출하여 UI의 **스냅숏**을 찍을 때 **고정**되었습니다.
...
하지만 리렌더링 전에 최신 상태를 읽고 싶다면 어떻게 해야 할까요? 다음 페이지에서 다루는 [상태 갱신 함수](./queueing-a-series-of-state-updates.mdx)를 사용할 수 있습니다!
